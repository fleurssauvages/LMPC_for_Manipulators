...
class LinearMPCController:
    ...
    def solve(self, ini_pose, des_pose, xi0 = None, obstacles=None):
        ...
        p_ref_world = sample_spline(spline, self.horizon, self.dt)

        if obstacles is not None and np.linalg.norm(ini_pose[:3,3] - des_pose[:3,3]) > 0.01:

            # --- convert reference + obstacles to local/tangent-ish coordinates ---
            R0 = ini_pose[:3,:3]
            p0 = ini_pose[:3,3]
            p_ref = (R0.T @ (p_ref_world - p0).T).T
